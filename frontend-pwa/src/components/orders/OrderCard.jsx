...
const OrderCard = ({ order, onUpdateStatus, isUpdating }) => {
  ...
  const formatTime = (timestamp) => {
    ...
    // La fecha ya viene como objeto Date desde OrdersManager
      ...
      <div className="p-4 space-y-3">
        ...
        {statusConfig.nextStatus && (
          <div className="pt-3 space-y-2">
            ...
            >
              {isUpdating ? <span className="flex items-center justify-center gap-2"><RefreshCw className="w-4 h-4 animate-spin" />Actualizando...</span> : statusConfig.nextLabel}
            </button>
            
            {order.status === 'pending' && (
              <button onClick={() => { if (window.confirm('¿Estás seguro de cancelar este pedido?')) { onUpdateStatus(order.id, 'cancelled'); } }} disabled={isUpdating} className="w-full py-2 bg-gray-100 text-gray-700 rounded-xl font-medium hover:bg-gray-200 transition-all disabled:opacity-50">
                Cancelar Pedido
              </button>
            )}
          </div>
