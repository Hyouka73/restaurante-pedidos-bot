...
module.exports = async (ctx) => {
  ...
  try {
    ...
    if (!session) {
      await ctx.answerCbQuery('Tu sesión de pedido ha expirado. Inicia uno nuevo con /pedido.');
      return;
    }
    ...
    if (callbackData.startsWith('add_item_')) {
      ...
      const order = await orderService.createOrder(session.restaurantId, orderData);
