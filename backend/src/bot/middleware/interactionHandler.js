...
// VERSIÓN FINAL CORREGIDA PARA REDIS
...
module.exports = async (ctx) => {
  ...
  try {
    ...
    if (callbackData.startsWith('add_item_')) {
      await handleAddItem(ctx, callbackData, userId, restaurantId);
      return;
    }
    ...
    if (callbackData.startsWith('qty_')) {
      await handleQuantityChange(ctx, callbackData, userId);
      return;
    }
    if (callbackData.startsWith('remove_')) {
      await handleRemoveItem(ctx, callbackData, userId);
      return;
    }
    if (callbackData === 'continue_to_delivery') {
      await handleContinueToDelivery(ctx, userId, restaurantId);
      return;
    }
    if (callbackData === 'delivery_yes') {
      await handleDeliveryYes(ctx, userId);
      return;
    }
    if (callbackData === 'pickup' || callbackData === 'change_to_pickup') {
      await handlePickup(ctx, userId, restaurantId);
      return;
    }
    if (callbackData.startsWith('payment_')) {
      await handlePaymentSelection(ctx, callbackData, userId, restaurantId);
      return;
    }
    if (callbackData === 'confirm_final') {
      await handleFinalConfirmation(ctx, userId, restaurantId);
      return;
    }
...
async function handleViewCart(ctx, userId) {
  ...
  session.items.forEach((item, index) => {
    ...
    cartMessage += `   ${item.quantity} x $${item.price} = *$${(item.price * item.quantity).toFixed(2)}*\n\n`;
  });
  cartMessage += '─'.repeat(25) + '\n';
...
async function handleQuantityChange(ctx, callbackData, userId) {
  const session = ctx.session?.cart;
  if (!session) {
...
async function handleRemoveItem(ctx, callbackData, userId) {
  const session = ctx.session?.cart;
  if (!session) {
    await ctx.answerCbQuery('⚠️ Sesión expirada');
    return;
...
async function handlePaymentSelection(ctx, callbackData, userId, restaurantId) {
  ...
  // ✅ CORRECCIÓN: Pasamos ctx en lugar de session
  await showFinalConfirmation(ctx, restaurantId);
}

// ✅ CORRECCIÓN: Esta función ahora obtiene session de ctx
async function showFinalConfirmation(ctx, restaurantId) {
  const session = ctx.session?.cart;
  if (!session) {
  ...
  session.items.forEach((item, i) => {
    confirmMessage += `${i + 1}. ${item.name} (${item.quantity}x) - $${(item.price * item.quantity).toFixed(2)}\n`;
  });
...
async function handleFinalConfirmation(ctx, userId, restaurantId) {
  ...
  try {
    ...
    // ✅ CORRECCIÓN: Usar delete en lugar de = null
